...
export const POST: RequestHandler = async ({ request }) => {
    try {
        ...
        if (!key || !contentType) {
            return new Response(JSON.stringify({ error: 'Key and contentType are required' }), { status: 400 });
        }
        ...
        return new Response(JSON.stringify({ error: 'Failed to generate pre-signed URL' }), { status: 500 });
    }
};